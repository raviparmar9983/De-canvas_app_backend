export const generateIdeationHtmlString = (ideation: any) => {
    return `
      <html lang="en">
        <head>
          <meta charset="UTF-8">
          ...
          <div class="ideation-container border border-2 border-dark" style="width: 1212px;">
            ...
            <div class="ideation-body">
              <div class="block block-1">
                <h5>People</h5>
                ${ideation.people.map((person: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${person}</p>`).join('')}
              </div>
              <div class="block block-2">
                <h5>Activity</h5>
                ${ideation.activity.map((act: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${act}</p>`).join('')}
              </div>
              <div class="block block-3">
                <h5>Situation/Context/Location</h5>
                ${ideation.context.map((ctx: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${ctx}</p>`).join('')}
              </div>
              <div class="block block-4">
                <h5>Props / Possible Solution</h5>
                ${ideation.props.map((prop: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${prop}</p>`).join('')}
              </div>
            </div>
          </div>
        </body>
