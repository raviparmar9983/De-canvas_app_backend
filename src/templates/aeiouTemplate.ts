export const generateHtmlString = (aeiou: any) => {
    return `
      <html lang="en">
        <head>
          <meta charset="UTF-8">
            ...
            <div class="aeiou-container border border-2 border-dark" style="width: 1212px;">
              ...
              <div class="aeiou-body">
                <div class="block block-1">
                  <h5>Environment</h5>
                  ${aeiou.environment.map((env: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${env}</p>`).join('')}
                </div>
                <div class="block block-2">
                  <h5>Interaction</h5>
                  ${aeiou.intrection.map((intr: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${intr}</p>`).join('')}
                </div>
                <div class="block block-3">
                  <h5>Objects</h5>
                  ${aeiou.object.map((obj: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${obj}</p>`).join('')}
                </div>
                <div class="block block-4">
                  <h5>Activities</h5>
                  ${aeiou.activity.map((act: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${act}</p>`).join('')}
                </div>
                <div class="block block-5">
                  <h5>Users</h5>
                  ${aeiou.user.map((usr: string, i: number) => `<p class="bg-warning p-1 ptag" id="${i}">${usr}</p>`).join('')}
                </div>
              </div>
            </div>
          </div>
